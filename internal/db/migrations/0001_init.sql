-- +goose Up
-- Core state graph. See docs/design-spec.md §2 (Data Model) and §3 (Storage).
...
-- A label is a unique alias for a state, like its id (docs/design-spec.md §2, §6).
...
-- executable is the one preserved permission bit (0 or 1); see docs/design-spec.md §2.
...
-- metadata the tree never depends on. Local, never synced. See docs/design-spec.md §2.
...
-- mtime is not older than recorded_at are never trusted. See docs/design-spec.md §4.
