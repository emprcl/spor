...
// newRerootCmd builds `spor reroot <ref>`, the dual of prune: it keeps only the
// target and its descendants, dropping everything else (docs/SPEC.md §5, §6). It
// is destructive, so it confirms first and reports exactly what will be dropped.
func newRerootCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			plan, err := eng.RerootPlan(ctx, ref)
			if err != nil {
				return err
			}
			out := cmd.OutOrStdout()
			if plan.IsNoop {
				fmt.Fprintf(out, "%s already contains the whole history; nothing to reroot.\n", abbrev(plan.Target))
				return nil
			}

			if !yes {
				fmt.Fprintf(out, "Rerooting to %s keeps %d %s and drops %d %s.\n",
					abbrev(plan.Target),
				...
				if !promptYesNo(cmd.InOrStdin(), out, "Reroot?") {
					fmt.Fprintln(out, "Aborted; nothing was dropped.")
					return nil
				}
			}

			res, err := eng.Reroot(ctx, ref)
			if err != nil {
				return err
			}
			fmt.Fprintf(out, "Rerooted to %s; dropped %d %s, kept %d.\n",
